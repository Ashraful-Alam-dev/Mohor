  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 flex flex-col transition-colors duration-300">
      ...
      <main className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-grow space-y-8">
        <div>
          <h1 className="text-2xl font-black tracking-tight uppercase text-neutral-950 dark:text-white">Your Cart Bags</h1>
          <p className="text-xs font-medium text-neutral-400 mt-0.5">Review items currently staged for purchase reservations.</p>
        </div>
        ...
        {cartLoading ? (
          ...
          <div className="border border-dashed border-neutral-200 dark:border-neutral-800 rounded-3xl p-16 text-center space-y-4 bg-white dark:bg-neutral-900">
            <p className="text-sm font-bold text-neutral-400">Your collection layout queue is completely empty.</p>
            <Link href="/" className="inline-block bg-brand-500 text-white text-xs font-bold px-6 py-3 rounded-xl uppercase tracking-wider">
          ...
          <div className="space-y-8">
              ...
              {cart.map((item) => {
                ...
                return (
                  <div 
                    ...
                    className="flex flex-col bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden shadow-sm p-3 relative group"
                  >
                    ...
                    <div className="mt-4 pt-3 border-t border-neutral-100 dark:border-neutral-800 space-y-3">
                      ...
                      <div className="flex items-center justify-between text-xs font-bold">
                        <span className="text-neutral-400 uppercase tracking-wide">Selected Bag Depth</span>
                        <span className="font-mono text-neutral-950 dark:text-white bg-neutral-100 dark:bg-neutral-800 px-2 py-0.5 rounded">
                          {item.selected_quantity} {item.selected_quantity === 1 ? 'unit' : 'units'}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center border border-neutral-200 dark:border-neutral-800 rounded-xl bg-neutral-50 dark:bg-neutral-950 overflow-hidden">
                          <button 
                            ...
                            onClick={() => updateQuantity(item.cart_item_id, item.selected_quantity - 1)}
                          ...
                          <button 
                            disabled={item.selected_quantity >= item.available_stock}
                            onClick={() => updateQuantity(item.cart_item_id, item.selected_quantity + 1)}
                            className="w-9 h-9 font-bold hover:bg-neutral-100 dark:hover:bg-neutral-900 transition text-sm text-neutral-600 dark:text-neutral-400 disabled:opacity-20 cursor-pointer"
                          >
                            +
            ...
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6">
              <div>
                <p className="text-xs font-black uppercase tracking-widest text-neutral-400">Total Purchase Weight</p>
                <h2 className="text-3xl font-black text-neutral-950 dark:text-white mt-1">৳ {cartTotal.toLocaleString()}</h2>
              </div>
              
              <button 
                onClick={() => alert('Proceeding to checkout management system...')}
